...
public class DataStore {
    ...
    public DataStore(String rawRecipePath) {
        ...
        SparkContext sparkCtx = DataTransformer.configureSpark(null, recipe.getContainer(K.SPARK));
        ...
        this.spark = SparkSession.builder()
                                 ...
                                 .config("spark.memory.offHeap.enabled", sparkCtx.getMemoryOffHeapEnabled())
                                 .config("spark.memory.offHeap.size", sparkCtx.getMemoryOffHeapSize())
                                 .master(sparkCtx.getMaster())
                                 .getOrCreate();

        // Register UDFs here
        this.spark.udf().register("genUuid", new UuidGeneratorUdf(), DataTypes.StringType);
    ...
    private void preprocessRecipe(String activeRecipePath) {
        ...
        if (Utils.existsInFile(activeRecipePath, K.IMPORT)) {
            ...
            String origFilename = Paths.get(rawRecipePath).getFileName().toString();
            String wipRecipePath = FileUtils.getTempDirectoryPath() + TEMP_FILE_PREFIX + UUID.randomUUID() + "_" +
                                   origFilename;

            log.debug("Original Recipe Filename: {}\nFile Path Created by Preprocessor: {}", origFilename,
                      wipRecipePath);
            ...
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(wipRecipePath));
                 BufferedReader reader = new BufferedReader(new FileReader(activeRecipePath))) {
                ...
                while ((line = reader.readLine()) != null) {
                    if (line.trim().startsWith(prefix)) {
                        ...
                        // should have the format of "$import: <some file path>". the path may have variables
                        if (importParts.length != 2) {
                            String errMsg = "The import tag has an invalid format: {}" + line;
                            log.error(errMsg);
                        ...
                        String fragmentPath = interpolator.replace(importParts[1].trim());
                        ...
                        try (BufferedReader fragReader = new BufferedReader(new FileReader(fragmentPath))) {
                            ...
                            while ((fragLine = fragReader.readLine()) != null) {
                                ...
                                if (!trimmedFragLine.isEmpty()) {
                                    embeddedFragmentCnt += (trimmedFragLine.startsWith(K.IMPORT) ? 1 : 0);
                            ...
                            String errMsg = "An error occurred while writing fragment lines to recipe file";
                            log.error(errMsg);
                            throw new FastLaneException(errMsg);
                        }
                    }
            ...
            if (activeRecipePath.contains(TEMP_FILE_PREFIX)) {
                ...
                try {
                    ...
                    log.error("An error occurred while deleting temp file {}.\nCause: {}", activeRecipePath,
                              e.getMessage());
                }
            }

            if (embeddedFragmentCnt > 0) {
                log.debug("Embedded Fragments: {}\nRecursively calling preprocess for: {}", embeddedFragmentCnt,
                          wipRecipePath);
    ...
    public DataStore add(String tableName, Dataset<Row> df) {
        ...
        if (df0 == null) {
            BigDecimal newDatapoints = new BigDecimal(df.columns().length).multiply(new BigDecimal(df.count()));
    ...
    public void writeToDeltaLake(String dataframeName, String location) {
        ...
        df.write().format("delta").option("overwriteSchema", true).mode("overwrite").save("/tmp/" + location);
    }
     ...
     */
    public void addConnection(String connectionType, String instanceName, ConnectionContext instanceContext) {
        if (StringUtils.isNotEmpty(connectionType) && StringUtils.isNotEmpty(instanceName) && instanceContext != null) {
            Map<String, ConnectionContext> instances = connectionStore.get(connectionType);
    ...
    public StructType getSchema(String schemaName) {
        return (StringUtils.isNotEmpty(schemaName) ? schemaStore.get(schemaName) : null);
    }
     ...
     * 
     * @param String
     *               connectionType - type of connection e.g. jdbc (represented
     *               by attribute tag)
     * @param String
     ...
     */
    public ConnectionContext getConnectionInstance(String connectionType, String instanceName) {
        ConnectionContext connectionInstance = null;

        if (StringUtils.isNotEmpty(connectionType) && StringUtils.isNotEmpty(instanceName)) {
            Map<String, ConnectionContext> instances = connectionStore.get(connectionType);
