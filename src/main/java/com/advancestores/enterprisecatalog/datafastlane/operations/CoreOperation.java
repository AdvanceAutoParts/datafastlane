...
public abstract class CoreOperation implements Operationable {
    private static final Logger log = LoggerFactory.getLogger(CoreOperation.class);
    ...
    protected ReaderContext buildReader(SparkSession session) {
        ...
        if (StringUtils.isEmpty(format)) {
            ...
            // if it uses a connection reference, look it up; if not a connection, assume file
            if (StringUtils.isNotEmpty(connectionName)) {
                ConnectionContext ctx = store.getConnectionInstance(format, connectionName);

                if (ctx != null) {
                    // map DFL properties to Spark-named properties - by default, the table to load
                    // is the same name as the dataframe; however, that can be overridden using the
                    // source option
                    String source = options.getOptionAsString("source");
                    reader = reader.option("dbtable", (StringUtils.isNotEmpty(source) ? source : dataframeName));
                    ...
                    log.debug("\nReader context for table " + dataframeName + " using connection reference " + format +
                              ":" + connectionName);
                }
                else {
                    throw new FastLaneException("Unable to find Connection reference '" + format + ":" +
                                                connectionName + "'.");
                }
            }
            else {
                // if the path is an s3 file, make a local copy before processing. the roleArn and region should
                // be defined as options. the destination path may also be specified as an option
                filePath = Utils.downloadS3File(operationDefinition.getPath(),
                                                options.getOptionAsString(K.KEY_DESTINATION),
                                                new AwsCredentialsContext(options.getOptionAsString(K.KEY_AWS_ACCESS_KEY_ID),
                ...
                log.debug("\nLoading file {} using path: {}\n", dataframeName, filePath);
            }

            // if a custom schema is required, pull it from the schema data store
            StructType schema = store.getSchema(operationDefinition.getSchemaName());

            if (schema != null) {
                reader = reader.schema(schema);
            }
            else {
                log.debug("Unable to find a schema object with name '" + operationDefinition.getSchemaName() +
    ...
    protected WriterContext buildWriter(Dataset<Row> dataframe) {
        ...
        // The path property identifies the destination of the save. if format is a file-type, such as csv,
        // it will be the actual path of the file to be created. If jdbc, it will be a table name. If Elasticsearch
        // it will be an index name
        if (StringUtils.isEmpty(destination)) {
            String errMsg = "The destination path was not specified. Cannot continue.";
            log.error(errMsg);
            throw new FastLaneException(errMsg);
        }
        ...
        boolean hasHeader = (StringUtils.isEmpty(headerStr) ? false : ("true".equals(headerStr.trim().toLowerCase())));
        ...
        if (StringUtils.isNotEmpty(connectionName)) {
            ...
            if (ctx != null) {
                log.debug("\nWriter context for table " + dataframeName + " using connection reference " + format +
                          ":" + connectionName);

                writer = dataframe.write().format(format);

                // jdbc wants dbtable for the table name - the value is stored in the destination option
                ...
                throw new FastLaneException("Unable to find Connection reference '" + format + ":" + connectionName +
                                            "'.");
            }
        }
        else {
            log.debug("\nWriter context for file {} of format {} will be coalesced into a single file.", dataframeName,
                      format);

            // this will be a file write...use coalesce to force a single file for mergable types
        ...
        // the dataframe is passed in because "save to text" requires some data manipulation
    ...
    public boolean isStore() {
        return (StringUtils.isEmpty(containerName) ? true : containerName.trim().toLowerCase().equals(K.STORE_NAME));
    }
