...
public class PivotGroupByOperation extends CoreOperation {
    private static final Logger log = LoggerFactory.getLogger(PivotGroupByOperation.class);
    ...
    public boolean run() {
        ...
        List<String> excludedColumnsFromRecipe = operationDefinition.getExcludingAsList();
        ...
        for (String colName : excludedColumnsFromRecipe) {
            if (colName.startsWith("*")) {
                ...
                if (colName.endsWith("*")) {
                    excludedColumnsStartsWith.add(colName.substring(0, colName.length() - 1));
                }
                else {
        ...
        log.debug("Excluded column starting with: {}", excludedColumnsStartsWith);
        ...
        for (String colName : df.columns()) {
            ...
            if (includeColumn && isExcludedEndsWith(excludedColumnsEndsWith, colName)) {
                includeColumn = false;
                firstOnlyColumns.add(first(df.col(colName)).as(colName));
            }
            if (includeColumn && isExcludedStartsWith(excludedColumnsStartsWith, colName)) {
                includeColumn = false;
                firstOnlyColumns.add(first(df.col(colName)).as(colName));
            }
            if (includeColumn) {
        ...
        df = df.orderBy(pivotColumnName)
               ...
               .agg(min(pivotColumnName).as(pivotColumnName), firstOnlyColumns.toArray(new Column[0]));
