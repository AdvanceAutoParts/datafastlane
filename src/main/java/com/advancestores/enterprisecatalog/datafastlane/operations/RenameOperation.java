...
public class RenameOperation extends CoreOperation {
  private static final Logger log =
      LoggerFactory.getLogger(RenameOperation.class);
  ...
  public boolean run() {
    ...
    // Get the operation definition, which is the whole block of the
    // YAML-based recipe file
    ...
    if (!SparkUtils.hasColumn(df, columnName)) {
      log.error(
          "Dataframe {} does not contain column {}, dataframe has columns: {}",
    ...
    if (to == null) {
      log.error(
          "Target name for renaming column '{}' is null, in dataframe '{}'",
          columnName, dataframeName);
      return false;
    }

    // Processing
    log.debug("About to rename column '{}' to '{}' in dataframe '{}'",
        columnName, to, dataframeName);
