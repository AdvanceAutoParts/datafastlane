...
public abstract class OperationBuilder {
    ...
    private static String OPERATION_CLASS_PACKAGE = "com.advancestores.enterprisecatalog.datafastlane.operations.";

    public static CoreOperation build(DataStore store, String attributeName, Operation operation) {
        return build(store, null, attributeName, operation);
    }

    public static CoreOperation build(DataStore store, String containerName, String attributeName,
                                      Operation operation) {
        ...
        try {
            ...
            log.error("Class {} could not be accessed: {}", className, e.getMessage());
            ;
            return null;
        }
        catch (InstantiationException e) {
            log.error("Class {} could not be instanciated: {}", className, e.getMessage());
            ;
            return null;
        }
        catch (ClassNotFoundException e) {
            log.error("Class {} could not be found: {}", className, e.getMessage());
            ;
            return null;
        }
    ...
    private static String getClassNameForOperation(String operationName) {
        ...
        if (operationName.contains(".") == true) {
            ...
            className = OPERATION_CLASS_PACKAGE + operationName.substring(0, 1).toUpperCase() +
