...
public class JoinOperation extends CoreOperation {
    ...
    public boolean run() {
        ...
        if (rightTableName == null) {
            log.error("You need to specify the right part of the join and the join column, skipping this step.");
            return false;
        }
        ...
        log.debug("About to join '{}'.'{}' with '{}'.'{}' using a '{}' join", tableName, columnName, rightTableName,
                  rightColumnName, joinType);
        Dataset<Row> rightDf = store.get(rightTableName);
        if (rightDf == null) {
            log.error("Could not find a dataframe called '{}' in the store, ignoring the join operation",
                      rightTableName);
            return false;
        }
        Dataset<Row> df = store.get(tableName);
        if (df == null) {
            log.error("Could not find a dataframe called '{}' in the store, ignoring the join operation", tableName);
            return false;
        }

        // Processing
        try {
            df = df.join(rightDf, df.col(columnName).equalTo(rightDf.col(rightColumnName)), joinType);
        }
        catch (Exception e) {
            log.error("An error occured while performing a join operation: {}", e.getMessage(), e);
