...
public class SelectOperation extends CoreOperation {
  private static final Logger log =
      LoggerFactory.getLogger(SelectOperation.class);
  ...
  public boolean run() {
    ...
    Dataset<Row> resDf =
        df.select(operationDefinition.getWithAsColumnArray());

    if (operationDefinition.getOptionValue(K.DISTINCT, "false")
        .toLowerCase().charAt(0) == 't') {
