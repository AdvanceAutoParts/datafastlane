...
/**
 * The Spark "sample" method does not take "percent" of the rows. Instead, the fraction value represents a "probability"
 * applied to each row. It uses Bernoulli sampling and the resulting row count will be different on each call.
 */
public class SampleOperation extends CoreOperation {
    private static final Logger log = LoggerFactory.getLogger(SampleOperation.class);
    ...
    public boolean run() {
        ...
        // monitor this in case row counting is taking an unexpected large amount of time
        long countStart = System.currentTimeMillis();

        rowCount = df.count();

        log.debug("Row count call completed in " + ((System.currentTimeMillis() - countStart) / 1000) +
