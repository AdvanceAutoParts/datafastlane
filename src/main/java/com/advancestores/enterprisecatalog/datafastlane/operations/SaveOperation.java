...
public class SaveOperation extends CoreOperation {
    private static final Logger log = LoggerFactory.getLogger(SaveOperation.class);
    ...
    public boolean run() {
        ...
        if (operationDefinition != null && store != null) {
            ...
            if (dataframe == null) {
                String errMsg = "Unknown dataframe '" + dataframeName + "' in store.";
                log.error(errMsg);
                throw new FastLaneException(errMsg);
            }
        ...
        log.debug("Run of " + this.getClass().getName() + " completed in " +
                  (System.currentTimeMillis() - runStart) / 1000 + " seconds with a status of " + retval);
