...
/**
 * Operation to change the letter case of a column to Initial Cap (first letter in everyword is capitalized)
 *
 * <pre>
 * letterCaseTest:
...
public class InitialCapOperation extends CoreOperation {
    private static final Logger log = LoggerFactory.getLogger(InitialCapOperation.class);
    ...
    public boolean run() {
        ...
        df = df.withColumn(operationColumnName, initcap(df.col(operationColumnName)));
