...
public class NestedGroupByOperation extends CoreOperation {
    private static final Logger log = LoggerFactory.getLogger(NestedGroupByOperation.class);
    ...
    public boolean run() {
        ...
        selectColumns.addAll(Arrays.asList(operationDefinition.getOnAsColumnArray()));
        selectColumns.addAll(Arrays.asList(operationDefinition.getNestingAsColumnArray()));
        String nestingColumn = operationDefinition.getNestingAsArray()[0];
        String structName = operationDefinition.getOptionValue(K.STRUCT_NAME, nestingColumn + K.NESTING_COLUMN_SUFFIX);
        String listName = operationDefinition.getOptionValue(K.LIST_NAME, nestingColumn + K.LIST_NAME_SUFFIX);
        Dataset<Row> resDf = df
                        .select(selectColumns.toArray(new Column[0])).distinct()
                        .withColumn(structName, struct(operationDefinition.getNestingAsColumnArray()))
                        .drop(operationDefinition.getNestingAsArray())
                        .groupBy(operationDefinition.getOnAsColumnArray()).agg(collect_list(structName).as(listName));
