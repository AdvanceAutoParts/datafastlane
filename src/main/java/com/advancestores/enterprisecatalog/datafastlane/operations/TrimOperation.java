...
public class TrimOperation extends CoreOperation {
    private static final Logger log = LoggerFactory.getLogger(TrimOperation.class);
    ...
    public boolean run() {
        ...
        String explicitTrimLeft = operationDefinition.getOptionValue("left", "invalid").toLowerCase();
        String explicitTrimRight = operationDefinition.getOptionValue("right", "invalid").toLowerCase();

        TrimType trimType = TrimType.ALL;

        if(explicitTrimLeft.substring(0,1).charAt(0) == 't' && explicitTrimRight.substring(0,1).charAt(0) != 't')
        {
            trimType = TrimType.LEFT;
        }
        else if (explicitTrimLeft.substring(0,1).charAt(0) != 't' && explicitTrimRight.substring(0,1).charAt(0) == 't')
        {
            trimType = TrimType.RIGHT;
        }
        else if (explicitTrimLeft.substring(0,1).charAt(0) == 'f' && explicitTrimRight.substring(0,1).charAt(0) == 'f')
        {
            Log.error("Cannot specify false for both left and right options for Trim operation", dataframeName);
            return false;
        }

        log.debug("Trim operation applied to {}, Trim Type: {}", operationColumnName, trimType.toString());
        switch (trimType)
        {
            case ALL:
                df = df.withColumn(operationColumnName, trim(df.col(operationColumnName)));
                break;
            case LEFT:
                df = df.withColumn(operationColumnName, ltrim(df.col(operationColumnName)));
                break;
            case RIGHT:
                df = df.withColumn(operationColumnName, rtrim(df.col(operationColumnName)));
                break;
        }
        ...
        log.debug("{} completed in {} seconds.", this.getClass().getSimpleName(),
