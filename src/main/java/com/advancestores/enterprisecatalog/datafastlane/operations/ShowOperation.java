...
public class ShowOperation extends CoreOperation {
  private static final Logger log =
      LoggerFactory.getLogger(ShowOperation.class);
  ...
  private boolean getTruncate() {
    String value =
        operationDefinition.getOptions().getOptionAsString(TRUNCATE);
    if (value == null) {
      return false;
    }
  ...
  private int getNumberOfRows() {
    ...
    Integer value =
        operationDefinition.getOptions().getOptionAsInteger(NUM_ROWS);
    if (value == null) {
      return DEFAULT_NUMBER_OF_ROWS;
    }
