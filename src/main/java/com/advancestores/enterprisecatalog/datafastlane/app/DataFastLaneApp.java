...
/**
 * Configure the application to process a DFL recipe file. The details of what will happen are specified in the recipe.
 * This includes connections (input/output), schemas, and operations.
 * 
 * The input arguments are:
 * o recipeFilename - name of recipe file to process
 * o -dryrun - optional flag to signify the recipe is to be processed, but the transformation is to be skipped
 */
public class DataFastLaneApp {
    private static final Logger log = LoggerFactory.getLogger(DataFastLaneApp.class);
    ...
    public static void main(String[] args) {
        ...
        // process command line arguments - the first argument that is not '-dryrun' will
        // be considered to be the recipe. check the balance of the args for '-dryrun'
        ...
        log.info("Command Line Recipe File: {}", cmdLineRecipe);
        log.info("is dryRun: {}", isDryRun);

        if (StringUtils.isEmpty(cmdLineRecipe)) {
            String errMsg = "ERROR: The recipe filename is a required parameter to the application.\n" +
                            "Usage: java -jar DataFastLaneApp-<version> <recipe-filename> [--dryrun]";
            System.out.println(errMsg);
        }
        else {
            log.debug("Command Line Recipe File after Interpolation: {}", cmdLineRecipe);

            String dest = Utils.getEnv(K.KEY_DESTINATION, System.getProperty(K.KEY_DESTINATION));
            AwsCredentialsContext ctx = new AwsCredentialsContext(Utils.getEnv(K.KEY_AWS_ACCESS_KEY_ID,
            ...
            String recipe = interpolator.replace(cmdLineRecipe); // check for variable interpolation
            ...
            if (recipe.startsWith(K.S3_PREFIX)) {
                ...
                // if the environment variables are not set, no need to continue - check required ones
                if (StringUtils.isEmpty(ctx.getAccessKeyId()) || StringUtils.isEmpty(ctx.getSecretAccessKey()) ||
                    StringUtils.isEmpty(ctx.getRegion())) {
                    String errMsg = "The AWS credentials are required before a file can be copied from S3. Check the enviornment variables.";
                    log.error(errMsg);
            ...
            try {
                log.info("DataFastLaneApp is starting and will process recipe: {}", recipe);
                ...
                if (!isDryRun) {
                    ...
                    log.info("***** This is a dryrun. Transformation not executed. *****");
                }

                log.info("DataFastLaneApp completed in {} seconds with a status of {}.",
