...
public class Recipe {
    ...
    public void addDynamicProperty(String name, List<Attribute> steps) {
        ...
        if (getContainer(name) != null) {
            log.warn("Container with name '{}' already exists in recipe. Adding container with duplicate name.", name);
        }
    ...
    /**
     * This will find the first container with the name provided. If multiple values of the same name exist, use
     * getContainers to get all containers with the same name.
     * 
     * @param String name - name of container for which to search
     * 
     * @return Container - the first container that matches the name provided
     */
    public Container getContainer(String name) {
        return containers.stream().filter(c -> c.getContainerName().equals(name)).findFirst().orElse(null);
    }
    ...
    public Container[] getContainers(String name) {
        return containers.stream().filter(c -> c.getContainerName().equals(name)).toArray(Container[]::new);
    }
