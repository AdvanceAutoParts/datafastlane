...
public abstract class SparkUtils {
    ...
    /**
     * Debugs a dataframe by displaying its schema, up to 10 rows of its content, and the row count. Counting rows may be an expensive operation.
     * 
     * @param df
     * @return
     */
        ...
        for (int i = 0; i < columns.length; i++) {
            ...
            if (fieldsToAnalyze == null || isInArray(fieldsToAnalyze, column)) {
                ...
                if (isNumericOrDate(type)) {
                    String min = df.orderBy(col(column)).first().getAs(column).toString();
                    String max = df.orderBy(col(column).desc()).first().getAs(column).toString();
                    log.debug("Min value: {} / max value: {}", min, max);
                }
                Dataset<Row> subDf = df.groupBy(col(column)).count();
                long subDfRowCount = subDf.count();
                log.debug("Distinct values for {}/{} ({}%)", subDfRowCount, rowCount,
    ...
    public static boolean isNumeric(DataType type) {
        if (type == DataTypes.DoubleType || type == DataTypes.FloatType || type == DataTypes.ShortType ||
