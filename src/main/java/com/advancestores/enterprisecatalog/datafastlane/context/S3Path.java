 ...
 * Example:
 * 
 * input: s3://test-data/recipes/fragments/recipe.yaml
 * output:
 * prefix = s3://
 * bucketName = test-data
 * path = /recipes/fragments/recipe.yaml
...
public class S3Path {
    ...
    /**
     * At a minimum, expects the format to be:
    ...
    private static boolean isValidFormat(String s3Path) {
        ...
        if (StringUtils.isEmpty(s3Path)) {
            ...
            log.error("S3 path string does not begin with '" + K.S3_PREFIX + "'.");
        }
        else if (StringUtils.countMatches(s3Path, "/") < 3) {
            log.error("S3 path string does not contain enough path separators, i.e. missing path components.");
        }
            ...
            log.error("S3 path string should not use triple consecutive path separators.");
        }
        else if (StringUtils.countMatches(s3Path, "//") > 1) {
            log.error("S3 path string should not use multiple consecutive path separators.");
        }
        else {
