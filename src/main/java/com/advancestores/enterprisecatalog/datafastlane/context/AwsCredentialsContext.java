...
/**
 * Manages credential properties used by an AWS clients. The properties/options are read from
 * the recipe file where AWS S3 file access is used.
 * 
 * Temporary session credentials can be generated using the following command:
 * 
 * aws sts get-session-token --serial-number <user ARN> --token-code <mfa code> --profile <profile tag from
 * <homeDir>/.aws/credentials file>
 * 
 * Example using the following content from $HOME/.aws/credentials file:
 * ...
 * [default-long-term]
 * aws_access_key_id = ACCCUBAXYBXYXYXYXYXIQRMH7Q
 * aws_secret_access_key = 693CwQUhnXK3KD8FSDFNK28CCCC+pg1vNKne1zsDIioX
 * ...
 * 
 * "aws sts get-session-token \
 * --serial-number arn:aws:iam::942075117532142:mfa/john.doe \
 * --token-code 568942 \
...
public class AwsCredentialsContext extends BaseContext {
    ...
    public AwsCredentialsContext(String accessKeyId, String secretAccessKey, String sessionToken, String region,
                                 String roleArn) {
        super();
