...
/**
 * Stores information surrounding a DataFrameReader. The goal was to use the DataFrameReader options map to store extra
 * things; however, the property is private. Things can be put in, but not read out.
 */
public class ReaderContext {
    private static final Logger log = LoggerFactory.getLogger(ReaderContext.class);
    ...
    public Dataset<Row> load() {
        ...
        try {
            if (filePath == null) {
                log.info(">>>> Loading {} using connection reference {}", dataframeName, connectionName);
                dataframe = reader.load();
            }
            else {
                log.info(">>>> Loading {} from file {}", dataframeName, filePath);
                dataframe = reader.load(filePath);
            }

            log.info(">>>> Load completed in {} seconds", ((System.currentTimeMillis() - loadStart) / 1000));
        }
        catch (Exception e) {
